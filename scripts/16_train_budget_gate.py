...
def build_cache_batch(cache: dict, query_cfg: dict, coverage_cfg: dict, device: str):
    hidden = cache["hidden"].to(device=device, dtype=torch.float32)
    ...
    return h, query_aux, coverage_aux, gains, query_hidden
...
def score_with_bases(
    ...
    return query_scores, coverage_scores
...
def main() -> None:
    ...
    for epoch in range(epochs):
        ...
        for idx in tqdm(perm, desc=f"epoch {epoch}"):
            ...
            h, query_aux, coverage_aux, gains, query_hidden = build_cache_batch(
                cache,
                query_cfg,
                coverage_cfg,
                device,
            ...
            query_scores, coverage_scores = score_with_bases(
                h,
                query_aux,
                coverage_aux,
                query_hidden,
            ...
            for budget in budgets:
                ...
                scores, gate_values = gated_scores(gate, query_scores, coverage_scores, features)
                ...
                total_gate[budget].append(float(gate_values.mean().item()))
        ...
        gate_means = " ".join(
            f"gate@{k}={sum(total_gate[k]) / max(len(total_gate[k]), 1):.3f}" for k in budgets
        )
        metric_parts = []
        for budget in budgets:
        ...
        print(
            ...
            f"{gate_means} {' '.join(metric_parts)}"
        )
