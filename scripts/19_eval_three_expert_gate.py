...
def score_gate_candidates(
    ...
    with torch.no_grad():
        ...
        scores, _ = softmax_mixture_scores(
            gate,
...
def main() -> None:
    ...
    parser.add_argument("--gate_label", default="g1b_gate")
    ...
    legacy_cfg = load_yaml(gate_cfg["gate"]["legacy_config"])
    ...
    for sample_idx, sample in enumerate(tqdm(samples, desc="three-expert gate eval")):
        try:
            ...
            print(f"[WARN] three-expert gate eval sample failed: {exc}")
            continue
