...
def download_vg_image(image_id: str, output_path: str, timeout: int = 30) -> bool:
    ...
    Download only the small subset needed for this jsonl instead of the full
    GQA images.zip.
    """
...
def main():
    ...
    parser.add_argument("--download_missing_images_from_vg", action="store_true")
        ...
        with open(args.output, "w", encoding="utf-8") as out:
            for qid, q in items:
                ...
                if args.image_zip:
                    ...
                    if args.download_missing_images_from_vg and not os.path.exists(image_path):
                        if not download_vg_image(image_id, image_path):
                            skipped += 1
                            continue
                else:
